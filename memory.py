...
class FailureInjectorApp:
    def __init__(self, master, registration):
        ...
        # Instance variables for current settings
        self.failure_probability = self.probability_var.get()
    ...
    def record_failure_triggered(self, failure_type):
        ...
        # Also update failures dict with thread safety
        with self.failures_lock:
            if self.registration not in self.failures_dict:
                self.failures_dict[self.registration] = set()
            self.failures_dict[self.registration].add(failure_type)
